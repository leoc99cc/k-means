...
class KMeans:
    ...
    def __init__(self, n_clusters=3, tolerance=0.01, max_iter=100, runs=1, init_method="forgy", dis_method='euclidean'):
    ...
    def fit(self, X):
        ...
        for i in range(self.runs):
            ...
            for _ in range(self.max_iter):
                ...
                cluster_means, sse, number = self.__compute_means(X_values, X_labels, col_count)
                SSE.append(sse)
        ...
        return SSE, number #all_clusterings[best_clustering_index]
    ...
    def __initialize_means(self, X, row_count):
        ...
        elif self.init_method == 'c1':
            return init_methods.c1(X, self.n_clusters)
        elif self.init_method == 'c2':
            return init_methods.c2(X, self.n_clusters)
        else:
    ...
    def __compute_distances(self, X, cluster_means, row_count):
        ...
        for cluster_mean_index, cluster_mean in enumerate(cluster_means):
            if self.dis_method == 'euclidean':
                distances[:, cluster_mean_index] = self.__euclidean(X, cluster_mean)
            elif self.dis_method == 'manhattan':
                distances[:, cluster_mean_index] = self.__manhattan(X, cluster_mean)
    ...
    def __compute_means(self, X, labels, col_count):
        ...
        for cluster_mean_index, _ in enumerate(cluster_means):
            ...
            number.append(len(cluster_elements))
            if len(cluster_elements):
                cluster_means[cluster_mean_index, :] = cluster_elements.mean(axis=0)
                if self.dis_method == 'euclidean':
                    SSE += np.sum(np.square(self.__euclidean(cluster_elements, cluster_means[cluster_mean_index])))
                elif self.dis_method == 'manhattan':
                    SSE += np.sum(self.__manhattan(cluster_elements, cluster_means[cluster_mean_index]))
        return cluster_means, SSE, number
    ...
    def __euclidean(self, X, cluster_mean):
        return np.linalg.norm(X - cluster_mean, axis=1)

    def __manhattan(self, X, cluster_mean):
        return np.linalg.norm(X - cluster_mean, ord=1, axis=1)
